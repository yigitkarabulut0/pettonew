...
export function computePlaydateState(
  ...
  // v0.11.0 — use the pet-level `slotsUsed` when the server provided a
  // real count, otherwise fall back to the legacy user-level attendees
  // length so freshly-created playdates still show "1 / N" immediately
  // after the creator auto-joins (before the list refetch lands).
  const slotsUsed =
